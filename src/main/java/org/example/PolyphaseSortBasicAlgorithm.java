...
// Ось цей виклик — єдиний правильний на зараз
        ...
        int phase = 0;
        while (true) {
            int activeFiles = countNonZero(runCounts);
            if (activeFiles < 2) break;  // залишилася одна серія — вихід
        */
/*while (countNonZero(runCounts) > 1) {
            ...
            // перерозподіл серій у два джерела
            int[] redistributed = distributeRuns(target, source1, source2, mergedRuns);

            */
/*runCounts[source1Idx] = redistributed[0];
            runCounts[source2Idx] = redistributed[1];
            //runCounts[targetIdx] = 0;
            runCounts[targetIdx] = mergedRuns; // target отримує результат злиття
*//*


            runCounts[source1Idx] -= mergedRuns;
            runCounts[source2Idx] -= mergedRuns;
...
*//*
    ...
    private static int[] chooseFilesToMerge(int[] counts) {
        ...
        if (min1 == -1 || min2 == -1) {
            throw new IllegalStateException("Недостатньо активних файлів для злиття");
        }
...
*//*
    ...
    */
/*private static int[] distributeInitialRuns(int chunkSize, String[] tempFiles) {
        int[] runCounts = new int[3];  // 0 = temp1, 1 = temp2, 2 = temp3 (0 спочатку)
        long totalRunsEstimate = (totalLines + chunkSize - 1) / chunkSize;
        long[] fib = getFibonacciDistribution(totalRunsEstimate);
        long targetForTemp1 = fib[0]; // більше
        long targetForTemp2 = fib[1]; // менше
        long writtenTo1 = 0;
        ...
        try (Scanner reader = new Scanner(new File("input.txt"));
             PrintWriter w1 = new PrintWriter(new FileWriter(tempFiles[0]));
             PrintWriter w2 = new PrintWriter(new FileWriter(tempFiles[1]))) {
            ...
            while (reader.hasNextLine()) {
                ...
                PrintWriter target = (writtenTo1 < targetForTemp1) ? w1 : w2;

                for (String line : chunk) {
                    target.println(line);
                }
                target.println(); // маркер серії
            ...
            // dummy-серії для підтримки Фібоначчі
            long dummies = (targetForTemp1 + targetForTemp2) - totalRuns;
            for (long i = 0; i < dummies; i++) {
                w1.println();
                runCounts[0]++;
...
*//*
    ...
    private static int[] distributeInitialRuns(int chunkSize, String[] tempFiles) {
        ...
        try (Scanner reader = new Scanner(new BufferedReader(new FileReader("input.txt")));
             PrintWriter w1 = new PrintWriter(new BufferedWriter(new FileWriter(tempFiles[0])));
             PrintWriter w2 = new PrintWriter(new BufferedWriter(new FileWriter(tempFiles[1])))) {

            int fileIdx = 0;
            PrintWriter current = w1;
            ...
            while (reader.hasNextLine()) {
                ...
                // Записуємо весь відсортований шматок як одну серію
                for (String line : chunk) {
                    current.println(line);
                }
                current.println();  // маркер кінця серії

                if (fileIdx == 0) runCounts[0]++;
                else              runCounts[1]++;

                totalRuns++;
                fileIdx = 1 - fileIdx;
                current = (fileIdx == 0) ? w1 : w2;
            }

            System.out.printf("Створено %d початкових серій%n", totalRuns);
        } catch (IOException e) {
            e.printStackTrace();
        }
...
/*private static int[] distributeRuns(String fromFile, String to1, String to2, int totalRunsInFrom) {
        ...
        try (Scanner r = new Scanner(new BufferedReader(new FileReader(fromFile)));
             PrintWriter w1 = new PrintWriter(new BufferedWriter(new FileWriter(to1)));
             PrintWriter w2 = new PrintWriter(new BufferedWriter(new FileWriter(to2)))) {

            while (r.hasNextLine()) {
                PrintWriter out = (written < targetTo1) ? w1 : w2;
                boolean hasData = false;

                String line;
                while (r.hasNextLine() && !(line = r.nextLine()).isEmpty()) {
                    out.println(line);
                    hasData = true;
                }
                ...
                if (hasData) {
                    out.println();  // маркер
                    if (written < targetTo1) newCounts[0]++;
                    else                     newCounts[1]++;
                    written++;
                }
            }
...
*//*
    ...
    private static int[] distributeRuns(String fromFile, String to1, String to2, int totalRunsInFrom) {
        ...
        long[] fib = getFibonacciDistribution(totalRunsInFrom);
        long targetTo1 = fib[0]; // більший файл
        long targetTo2 = fib[1]; // менший файл
        ...
        try (Scanner r = new Scanner(new BufferedReader(new FileReader(fromFile)));
            ...
            while (r.hasNextLine()) {
                String line = r.nextLine();
                if (line.isEmpty()) continue; // ігноруємо старі маркери

                char key = line.charAt(0);

                // якщо ключ зменшується → нова серія
                if (prevKey != 0 && key < prevKey) {
                    // закінчуємо серію, додаємо маркер
                    PrintWriter currentWriter = (writtenTo1 < targetTo1) ? w1 : w2;
                    currentWriter.println(); // маркер кінця серії

                    if (writtenTo1 < targetTo1) writtenTo1++;
                    else writtenTo2++;
                }
            ...
            // додаємо маркер для останньої серії
            if (prevKey != 0) {
                PrintWriter currentWriter = (writtenTo1 < targetTo1) ? w1 : w2;
            ...
            // додаємо dummy-серії для збереження Фібоначчі
            long dummiesNeeded = (targetTo1 + targetTo2) - (writtenTo1 + writtenTo2);
            for (long i = 0; i < dummiesNeeded; i++) {
                w1.println();
                writtenTo1++;
            }

            newCounts[0] = (int) writtenTo1;
            newCounts[1] = (int) writtenTo2;
...
public class PolyphaseSortBasicAlgorithm {
    ...
    private static int[] chooseFilesToMerge(int[] counts) {
        // У polyphase завжди зливаємо два файли з найменшою кількістю серій
        // у файл з найбільшою кількістю серій
        ...
        // Цільовий файл це той, який не є min1 і не є min2
        int target = 3 - min1 - min2;
